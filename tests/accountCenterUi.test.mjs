...
test('account center state supports profile switching and desktop-only auth actions', async () => {
  ...
  assert.match(state, /resolveDisplayedActiveProfileId\(/)
  assert.match(state, /connectionMode === 'shared'/)
  assert.match(state, /profile\.id === 'default'/)
  assert.doesNotMatch(state, /const supportsAccountProfiles = computed\(\(\) => serverConnectionMode\.value === 'isolated'\)/)
  ...
  assert.doesNotMatch(state, /共享模式下不支持切换账号档案/)
...
test('app mounts account center sheet with profile switching actions', async () => {
  ...
  assert.doesNotMatch(overview, /const showProfileList = computed\(\(\) => props\.serverConnectionMode === 'isolated'/)
  assert.doesNotMatch(overview, /v-if="!isMobileClient && serverConnectionMode === 'isolated'"/)
  assert.match(overview, /profile\.email/)
  assert.match(overview, /profile\.hasAuth/)
  assert.match(overview, /const showSwitchToIsolatedAction = computed\(\(\) =>/)
  assert.match(overview, /switch-to-isolated/)
  assert.doesNotMatch(overview, /sharedCodexAppHint/)
  assert.doesNotMatch(overview, /sharedModeFailureHint/)
  assert.match(picker, /serverConnectionMode: UiServerConnectionMode/)
  assert.match(picker, /canCreateProfileDuringLogin/)
  assert.doesNotMatch(picker, /props\.serverConnectionMode === 'isolated'/)
  assert.doesNotMatch(picker, /app\.accountCenterSharedModeHint/)
