...
- 账号池与切换能力通过私有 RPC `web-local/account/*` 提供，入口位于 Web/移动端侧边栏账号切换器。
- 已入池账号可直接切换为活跃账号，无需再次 OAuth 回调。
- 新账号首次入池仍依赖已有授权来源（`chatgptAuthTokens`）。
- 切换后，线程选择、滚动状态、context 用量按账号分桶恢复。
