...
test('ThreadComposer keeps mobile controls stable when the input is focused', async () => {
  ...
  assert.match(
    composer,
    /thread-composer-controls[\s\S]*thread-composer-config-group[\s\S]*thread-composer-main-controls[\s\S]*thread-composer-status-group[\s\S]*thread-composer-action-group/,
  )
  ...
  assert.match(composer, /thread-composer-mobile-status-button/)
  assert.match(composer, /thread-composer-mobile-status-button[\s\S]*:data-level="mobileStatusLevel"/)
  assert.match(composer, /thread-composer-actions-trigger[\s\S]*width:\s*2\.25rem;/)
  assert.match(composer, /thread-composer-actions-trigger[\s\S]*height:\s*2\.25rem;/)
  assert.match(composer, /thread-composer-voice-button[\s\S]*width:\s*2\.25rem;/)
  assert.match(composer, /thread-composer-submit[\s\S]*width:\s*2\.25rem;/)
  ...
  assert.match(composer, /thread-composer-main-controls[\s\S]*justify-content:\s*flex-start;/)
  assert.match(composer, /thread-composer-submit[\s\S]*min-width:\s*2\.25rem/)
  assert.match(composer, /thread-composer-stop[\s\S]*min-width:\s*2\.25rem/)
  assert.match(composer, /thread-composer-action-group[\s\S]*@apply\s+flex\s+items-center\s+gap-2;/)
  assert.match(composer, /thread-composer-status-group[\s\S]*@apply\s+flex\s+items-center\s+gap-1\.5;/)
  assert.match(composer, /thread-composer-controls[\s\S]*flex-direction:\s*row;/)
  assert.match(composer, /thread-composer-controls[\s\S]*justify-content:\s*flex-start;/)
  assert.match(composer, /thread-composer-main-controls[\s\S]*width:\s*auto;/)
  ...
  assert.match(composer, /thread-composer-branch-button[\s\S]*min-height:\s*2\.25rem;/)
